...
type Product = {
  ...
  product_variants: Array<{ price: string; currency: string; position: number }>
}
...
function ProductPicker({
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/products?brand_id=${encodeURIComponent(brandId)}&search=${encodeURIComponent(search)}&limit=8`,
        )
  ...
  const firstPrice = (p: Product) => {
    const v = [...p.product_variants].sort((a, b) => a.position - b.position)[0]
    return v ? `${v.currency} ${parseFloat(v.price).toFixed(2)}` : null
  }
    ...
    return (
      <div className="flex items-center gap-3 p-2.5 bg-gray-50 border border-gray-200 rounded-lg">
        {selected.featured_image_url ? (
          ...
          <div className="w-10 h-10 bg-gray-200 rounded border border-gray-100 shrink-0 flex items-center justify-center text-gray-400 text-xs">
            IMG
          </div>
        )}
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-gray-900 truncate">{selected.title}</p>
          <p className="text-xs text-gray-400">{firstPrice(selected) ?? selected.handle}</p>
        </div>
        <button
          ...
          {results.map((p) => (
            ...
            >
              {p.featured_image_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={p.featured_image_url} alt="" className="w-8 h-8 object-cover rounded border border-gray-100 shrink-0" />
              ) : (
                <div className="w-8 h-8 bg-gray-200 rounded shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-900 truncate">{p.title}</p>
                <p className="text-xs text-gray-400">{firstPrice(p) ?? p.handle}</p>
              </div>
            </button>
          ))}
        </div>
...
export function ContentStudio({ brandId = 'plasmaide' }: { brandId?: string }) {
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <form onSubmit={handleGenerate} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1.5">
              Featured product <span className="text-gray-400 font-normal">(optional — adds product context to the generation prompt)</span>
            </label>
            <ProductPicker
              brandId={brandId}
