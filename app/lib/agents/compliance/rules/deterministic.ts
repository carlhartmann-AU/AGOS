...
export function evaluateLengthCheck(
  ...
  // Skip if this rule is scoped to specific content types and current type doesn't match
  if (rule.content_types && contentType && !rule.content_types.includes(contentType)) {
    return {
      rule_id: rule.id,
      rule_name: rule.name,
      severity: rule.severity,
      passed: true,
      explanation: `Skipped — rule applies to ${rule.content_types.join(', ')} only.`,
    }
  }
